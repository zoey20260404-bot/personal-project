...
1. **数据层**：RAG 检索增强/结构化数据优先——模型负责"理解和表达"，不负责"记忆事实"
2. **Prompt 层**：禁令（"禁止编造"）+ 来源标注——辅助层，光靠 Prompt 约束不住小模型
3. **工具层**：事实走 Function Calling 查库，数字不许模型自己写
4. **校验层**：LLM 输出当"不可信输入"，代码规则兜底（JSON Schema、枚举白名单、引用 ID 回查）——本项目实战最多的一层
5. **评审层**：Reviewer 模型复核 / 高风险人工确认（human-in-the-loop）
6. **观测层**：TraceID 全链路记录 + bad case 收集进评测集 + 准确率监控

本项目对照：工具层 ✅（query_positions 取数）、校验层 ✅（置信度重算/省份归一/冲突检测/ID 真实性校验）、Prompt 层 ✅、观测层 ✅（TraceID）、评审层 ⚠️（need_confirm 人工确认有，Reviewer 自动校验 TODO）、数据层 ⚠️（SQL 取数有，知识库 RAG 未做）。

心法：**不信任模型的任何输出，但充分利用它的理解力**——事实给数据库管、判定给规则管、生成给模型管、关键决策给人管。
